...
def secret():return re.search(r'^ACCOUNT_3_GENLAYER_PRIVATE_KEY\s*=\s*"?([^"\r\n]+)',ENV,re.M).group(1).strip()
def address(value):
...
def main():
 ...
 code=(ROOT/"study/assessment.py").read_text()
 ...
 (ROOT/"runs/deployment.json").parent.mkdir(parents=True,exist_ok=True)
 (ROOT/"runs/deployment.json").write_text(json.dumps(out,indent=2))
